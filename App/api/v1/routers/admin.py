...
async def get_database_status():
    ...
    # Test local connection (blocking I/O → run in threadpool)
    try:
        def _test_local():
            with engine.connect():
                pass
        await run_in_threadpool(_test_local)
        ...
        try:
            def _test_supabase():
                with supabase_engine.connect():
                    pass
            await run_in_threadpool(_test_supabase)
    ...
    return DBStatusResponse(
        ...
        local_db_url=_mask_password(settings.DATABASE_URL),
        supabase_db_url=_mask_password(settings.SUPABASE_DB_URL) if settings.SUPABASE_DB_URL else None
    )


@router.post("/db-mode", response_model=dict)
async def update_database_mode(config: DBModeUpdate):
    """
    Update database mode at runtime.
    WARNING: This changes global configuration – use cautiously in production.
    """
